-- Migration: 2026-06-17 clinic workflow enhancements
-- Adds new columns to medical_records and creates tables for prescriptions, service_tickets, and invoices.
...
CREATE TABLE IF NOT EXISTS public.service_tickets (
  ...
  technician_id text, -- ID of technician who performs it
  result_notes text,
...
CREATE TABLE IF NOT EXISTS public.invoices (
  ...
  payment_method text, -- CASH, PAYOS
  transaction_id text, -- From PayOS if any
  created_at timestamptz DEFAULT now(),
  paid_at timestamptz
);
...
-- Permissive policies for now matching the anon/authenticated access
DO $$
BEGIN
