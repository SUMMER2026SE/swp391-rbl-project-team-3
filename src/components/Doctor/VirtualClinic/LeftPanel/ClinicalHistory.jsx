...
import { Activity, Beaker, FileText, CalendarDays } from 'lucide-react';
...
export default function ClinicalHistory({ patientId }) {
  ...
  if (patientLabs.length === 0 && patientRecords.length === 0) {
    return (
      <div className="glass-3d water-refract rounded-[2rem] p-8 text-center text-slate-500 font-medium mt-6">
  ...
  return (
    <div className="glass-3d water-refract rounded-[2rem] p-6 mt-6">
      <div className="flex items-center gap-2 mb-6 pb-4 border-b border-slate-200/40">
        <Activity className="w-5 h-5 text-teal-600" />
        <h3 className="font-extrabold text-lg text-slate-900">Lịch sử Cận lâm sàng & Tiến triển</h3>
      </div>
